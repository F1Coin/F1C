...
*Setup instructions for a Gitian build of F1coin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the F1coin
...
- [Building F1coin](#building-f1coin)
...
Clone the git repositories for f1coin and Gitian and then checkout the f1coin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/f1coin-project/f1coin.git
cd f1coin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the f1coin repository
...
To build F1coin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the f1coin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/f1coin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/f1coin-project/f1coin
...
For example:
```bash
URL=https://github.com/user/f1coin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit f1coin=${COMMIT} --url f1coin=${URL} ../f1coin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit f1coin=${COMMIT} --url f1coin=${URL} ../f1coin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit f1coin=${COMMIT} --url f1coin=${URL} ../f1coin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/f1coin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/f1coin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/f1coin-build.assert
```
...
[f1coin/gitian.sigs](https://github.com/f1coin-project/gitian.sigs.f1c/) repository, or if that's not possible create a pull
request. 
